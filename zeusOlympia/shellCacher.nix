...
{
  options = {
    shellCacher.shell = pkgsLib.mkOption {
    };
  };
  config.systemd.user.services."shellCacher-${config.shellCacher.shell.name}" = {
    ...
    serviceConfig = {
      ...
      ExecStart = pkgs.writeShellScript "cache${config.shellCacher.shell.name}Shell" ''
        export PATH="/run/current-system/sw/bin:$PATH"
        exec nix build ${config.shellCacher.shell.inputDerivation} --no-link --print-out-paths
      '';
